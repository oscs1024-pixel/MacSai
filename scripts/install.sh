#!/bin/bash
# One-line installer for Mac Clean.
# Usage: curl -fsSL https://raw.githubusercontent.com/iliyami/MacClean/main/scripts/install.sh | bash

set -euo pipefail

REPO="iliyami/MacClean"
APP_NAME="Mac Clean.app"
...
if [ "$OS_VERSION" -lt 14 ]; then
    red "Mac Clean requires macOS 14 (Sonoma) or later. You have $(sw_vers -productVersion)."
    exit 1
fi
...
cyan "Downloading Mac Clean $VERSION..."
TMP=$(mktemp -d)
...
cyan "Removing Gatekeeper quarantine flag..."
xattr -dr com.apple.quarantine "$INSTALL_DIR/$APP_NAME" 2>/dev/null || true

green ""
green "✓ Mac Clean $VERSION installed to $INSTALL_DIR/$APP_NAME"
green ""
